...
% scale_ft = mean(Ref_Sized(8e5:8.5e5))/mean(yfit(8e5:8.5e5)); % OLD
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(8.82e5:8.9e5))); % For Attack 1 Model (Clean Part)
yfit = yfit * scale_ft;
...
scale_ft = 0.3/mean(abs(yfit(8.82e5:8.9e5))); % For Attack 1 Model (Atk1)
yfit = yfit * scale_ft;

scale_ft = 0.3/mean(abs(x(8e5:8.5e5)));
x = x * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(z(8e5:8.5e5)));
z = z * scale_ft;
% ---- R-Square Calculation ----
yresid = z - yfit;
SSresid = sum(yresid.^2);
...
scale_ft = 0.3/mean(abs(w(8.82e5:8.9e5))); % For Attack 1 Model (Atk1)
w = w * scale_ft;
scale_ft = 0.3/mean(abs(x(8e5:8.5e5)));
x = x * scale_ft;
