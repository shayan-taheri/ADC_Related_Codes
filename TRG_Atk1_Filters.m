...
load('C:\Users\shaya\Desktop\Delta Sigma ADC\Code\TRG\Atk1.mat');
...
% scale_ft = mean(Ref_Sized(1.7e5:1.9e5))/mean(yfit(1.7e5:1.9e5)); % OLD
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(1.9e5:2.1e5))); % For Attack 1 Model (Clean Part)
yfit = yfit * scale_ft;

scale_ft = 0.3/mean(abs(x(1.7e5:1.9e5)));
x = x * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(1.9e5:2.1e5))); % For Attack 1 Model (Atk1)
yfit = yfit * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(z(1.7e5:1.9e5)));
z = z * scale_ft;
% ---- R-Square Calculation ----
yresid = z - yfit;
SSresid = sum(yresid.^2);
...
scale_ft = 0.3/mean(abs(w(1.9e5:2.1e5))); % For Attack 1 Model (Atk1)
w = w * scale_ft;
scale_ft = 0.3/mean(abs(x(1.7e5:1.9e5)));
x = x * scale_ft;
