...
load('C:\Users\shaya\Desktop\Delta Sigma ADC\Code\TRG\Atk2.mat');
...
% scale_ft = mean(Ref_Sized(1.9e5:2.1e5))/mean(yfit(1.9e5:2.1e5)); % OLD
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(1.8e5:1.9e5))); % For Attack 2 Model (Clean Part)
yfit = yfit * scale_ft;

scale_ft = 0.3/mean(abs(x(1.9e5:2.1e5)));
x = x * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(1.8e5:1.9e5))); % For Attack 2 Model (Atk2)
yfit = yfit * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(z(1.9e5:2.1e5)));
z = z * scale_ft;
% ---- R-Square Calculation ----
yresid = z - yfit;
SSresid = sum(yresid.^2);
...
scale_ft = 0.3/mean(abs(w(1.8e5:1.9e5))); % For Attack 2 Model (Atk2)
w = w * scale_ft;
scale_ft = 0.3/mean(abs(x(1.9e5:2.1e5)));
x = x * scale_ft;
