...
Atk2_Time = 1e6 * Atk2_Time;

% "Interpolation" for making the size of data vectors equal
% Ref_Sized = spline(Ref_Time,Ref_Vout,Atk2_Time); --> Not good for sizing 
Ref_Sized = interp1(Ref_Time,Ref_Vout,Atk2_Time); % Doing Interpolation

% Modified Residual: Reference Data - Filtered Data
% -------------------------------------------------------------------------

% General Filter Model: Atk2_Filtered = FUNCTION(Atk2_Vout);

% A) Simple Linear Regression
% Inputs: "Desired Data" (x) and "Test/Noisy Data" (y)
x = Ref_Sized; % Ref. Data in All Cases (except Case C)
y = Atk2_Vout; % Test/Noisy Data in All Cases
...
% scale_ft = mean(Ref_Sized(8.85e5:9.2e5))/mean(yfit(8.85e5:9.2e5)); % OLD
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(6.16e5:6.21e5))); % For Attack 2 Model (Clean Part)
yfit = yfit * scale_ft;

% Offset elimination again
scale_off = (0.3 - (0.267))/2;
yfit = yfit - scale_off;
...
scale_ft = 0.3/mean(abs(x(8.85e5:9.2e5)));
x = x * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(6.16e5:6.21e5))); % For Attack 2 Model (Atk2)
yfit = yfit * scale_ft;
...
y = Atk2_Vout; % Test/Noisy Data in All Cases
x = Atk2_Time; % Time Data in Case C
...
% Scaling all data
scale_ft = 0.3/mean(abs(z(8.85e5:9.2e5)));
z = z * scale_ft;
% ---- R-Square Calculation ----
yresid = z - yfit;
SSresid = sum(yresid.^2);
...
% Inputs: "Output Time" (Atk2_Time) and "Output Signal" (Atk2_Vout)
x = Ref_Sized; % Ref. Data in All Cases (except Case C)
y = Ref_Vs; % with "Ref_Time"
w = Atk2_Vout; % with "Atk2_Time"

y = interp1(Ref_Time,Ref_Vs,Atk2_Time); % Doing Interpolation

% Scaling all data before filtering
scale_ft = 0.3/mean(abs(w(6.16e5:6.21e5))); % For Attack 2 Model (Atk2)
w = w * scale_ft;
scale_ft = 0.3/mean(abs(x(8.85e5:9.2e5)));
x = x * scale_ft;

% Applying Filtering Process
for ind = 1:length(Atk2_Time)
    if (y(ind) >= 0.2)
        w(ind) = abs(min(y));
    else if (y(ind) <= -0.25)
        w(ind) = min(y);
    ...
    disp(['Completed Percentage = ',num2str(100*ind/length(Atk2_Time))]);
end
