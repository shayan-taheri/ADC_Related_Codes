...
% scale_ft = mean(Ref_Sized(8e5:8.4e5))/mean(yfit(8e5:8.4e5)); % OLD
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(8.15e5:8.21e5))); % For Attack 3 Model (Clean Part)
yfit = yfit * scale_ft;

% Offset elimination again
scale_off = (0.3 - (0.2502))/2;
yfit = yfit - scale_off;
...
scale_ft = 0.3/mean(abs(x(8e5:8.4e5)));
x = x * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(8.15e5:8.21e5))); % For Attack 3 Model (Atk3)
yfit = yfit * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(z(8e5:8.4e5)));
z = z * scale_ft;
% ---- R-Square Calculation ----
yresid = z - yfit;
SSresid = sum(yresid.^2);
...
scale_ft = 0.3/mean(abs(w(8.15e5:8.21e5))); % For Attack 3 Model (Atk3)
w = w * scale_ft;
scale_ft = 0.3/mean(abs(x(8e5:8.4e5)));
x = x * scale_ft;
