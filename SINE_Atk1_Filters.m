...
load('C:\Users\shaya\Desktop\Delta Sigma ADC\Code\SINE\Atk1.mat');

Ref_Time = 1e6 * Ref_Time;

Atk1_Time = 1e6 * Atk1_Time;

% "Interpolation" for making the size of data vectors equal
% Ref_Sized = spline(Ref_Time,Ref_Vout,Atk1_Time); --> Not good for sizing 
Ref_Sized = interp1(Ref_Time,Ref_Vout,Atk1_Time); % Doing Interpolation

% Modified Residual: Reference Data - Filtered Data
% -------------------------------------------------------------------------

% General Filter Model: Atk1_Filtered = FUNCTION(Atk1_Vout);

% A) Simple Linear Regression
% Inputs: "Desired Data" (x) and "Test/Noisy Data" (y)
x = Ref_Sized; % Ref. Data in All Cases (except Case C)
y = Atk1_Vout; % Test/Noisy Data in All Cases
...
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(1.10e6:1.12e6))); % For Attack 1 Model (Clean Part)
yfit = yfit * scale_ft;
...
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(1.10e6:1.12e6))); % For Attack 1 Model (Atk1)
yfit = yfit * scale_ft;
...
y = Atk1_Vout; % Test/Noisy Data in All Cases
x = Atk1_Time; % Time Data in Case C
...
% Inputs: "Output Time" (Atk1_Time) and "Output Signal" (Atk1_Vout)
x = Ref_Sized; % Ref. Data in All Cases (except Case C)
y = Ref_Vs; % with "Ref_Time"
w = Atk1_Vout; % with "Atk1_Time"

y = interp1(Ref_Time,Ref_Vs,Atk1_Time); % Doing Interpolation

% Scaling all data before filtering
scale_ft = 0.3/mean(abs(w(1.10e6:1.12e6))); % For Attack 1 Model (Atk1)
w = w * scale_ft;
scale_ft = 0.3/mean(abs(x(1.5e5:2.5e5)));
x = x * scale_ft;

% Applying Filtering Process
for ind = 1:length(Atk1_Time)
    if (y(ind) >= 0.2)
        w(ind) = abs(min(y));
    else if (y(ind) <= -0.25)
        w(ind) = min(y);
    ...
    disp(['Completed Percentage = ',num2str(100*ind/length(Atk1_Time))]);
end
