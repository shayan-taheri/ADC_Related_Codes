...
load('C:\Users\shaya\Desktop\Delta Sigma ADC\Code\SAW\All.mat');

Ref_Time = 1e6 * Ref_Time;

Atk4_Time = 1e6 * Atk4_Time;

% "Interpolation" for making the size of data vectors equal
% Ref_Sized = spline(Ref_Time,Ref_Vout,Atk4_Time); --> Not good for sizing 
Ref_Sized = interp1(Ref_Time,Ref_Vout,Atk4_Time); % Doing Interpolation

% Modified Residual: Reference Data - Filtered Data
% -------------------------------------------------------------------------

% General Filter Model: Atk4_Filtered = FUNCTION(Atk4_Vout);

% A) Simple Linear Regression
% Inputs: "Desired Data" (x) and "Test/Noisy Data" (y)
x = Ref_Sized; % Ref. Data in All Cases (except Case C)
y = Atk4_Vout; % Test/Noisy Data in All Cases
...
% scale_ft = mean(Ref_Sized(8.25e5:8.75e5))/mean(yfit(8.25e5:8.75e5)); % OLD
% Scaling all data
scale_ft = 0.3/mean(abs(yfit(8.35e5:8.39e5))); % For Attack 4 Model (Clean Part)
yfit = yfit * scale_ft;

% Offset elimination again
scale_off = (0.3 - (0.2485))/2;
yfit = yfit - scale_off;

% Scaling all data again
scale_ft = 0.3/mean(abs(yfit(8.35e5:8.39e5))); % For Attack 4 Model (Atk4)
yfit = yfit * scale_ft;

scale_ft = 0.3/mean(abs(x(8.25e5:8.75e5)));
x = x * scale_ft;
...
y = Atk4_Vout; % Test/Noisy Data in All Cases
x = Atk4_Time; % Time Data in Case C
...
% Scaling all data
scale_ft = 0.3/mean(abs(z(8.25e5:8.75e5)));
z = z * scale_ft;
% ---- R-Square Calculation ----
yresid = z - yfit;
SSresid = sum(yresid.^2);
...
% Inputs: "Output Time" (Atk4_Time) and "Output Signal" (Atk4_Vout)
x = Ref_Sized; % Ref. Data in All Cases (except Case C)
y = Ref_Vs; % with "Ref_Time"
w = Atk4_Vout; % with "Atk4_Time"

y = interp1(Ref_Time,Ref_Vs,Atk4_Time); % Doing Interpolation

% Scaling all data before filtering
scale_ft = 0.3/mean(abs(w(8.35e5:8.39e5))); % For Attack 4 Model (Atk4)
w = w * scale_ft;
scale_ft = 0.3/mean(abs(x(8.25e5:8.75e5)));
x = x * scale_ft;

% Applying Filtering Process
for ind = 1:length(Atk4_Time)
    if (y(ind) >= 0.2)
        w(ind) = abs(min(y));
    else if (y(ind) <= -0.25)
        w(ind) = min(y);
    ...
    disp(['Completed Percentage = ',num2str(100*ind/length(Atk4_Time))]);
end
